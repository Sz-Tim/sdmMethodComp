  ...
  if("agg" %in% process) {
    ...
    P_CAd <- lam.df %>% 
      ...
      mutate(prP=out$prP[,dim(out$prP)[2]],
             ...
             Surv.S.25.f=out$N_ad.25[,dim(out$N_ad.25)[2]], 
             Surv.S.75.f=out$N_ad.75[,dim(out$N_ad.75)[2]], 
             Surv.S.975.f=out$N_ad.975[,dim(out$N_ad.975)[2]], 
...
fit_IPM <- function(sp, sp_i, samp.issue, mod.issue, p, env.rct.unsc, lam.df, v,
  ...
  if("fit" %in% process) {
    p.c <- makeCluster(n_cores); registerDoSNOW(p.c)
    foreach(i=i_start:length(O_IPM), .errorhandling="pass", 
    ...
    if("CAi" %in% SDMs & !"IPM" %in% SDMs) {
      ...
      P_CAi <- lam.df %>%
        ...
        mutate(prP=out$Sf$prP,
               ...
               Surv.S.25.f=out$Sf$N_surv.25,
               Surv.S.75.f=out$Sf$N_surv.75,
      ...
      out <- list(Uf.pa=do.call("cbind", map(out.ls, ~.$Uf.pa)),
                  ...
                  lam.25=apply(lambdas, 1, quantile, probs=0.25),
                  lam.75=apply(lambdas, 1, quantile, probs=0.75),
      ...
      P_IPM <- lam.df %>%
        ...
        mutate(prP=out$prP,
               ...
               lambda.25.f=out$lam.25,
               lambda.75.f=out$lam.75,
