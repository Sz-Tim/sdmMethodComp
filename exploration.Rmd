...
sp_i <- list(p=list(s_z=c(-1.75, .75), # b1 + b2*z
                    ...
                    rcr_dir=0.8, # p(recruit directly)
...
B <- rpois(n.cell, n_i)
B_1 <- nSd <- D <- p_est.i <- rep(NA, n.cell)
...
for(i in 1:n.cell) {
  ...
  ### store seed production
  Fb[z.i,z.i,i] <- outer(y, y, calc_rcrDir, p=p, n_seedz=n_seedz, n_flz=n_flz, 
                         X.seed=X_seed[i,], X.fl=X_fl[i,]) *h * p$p_est
  Fb[1,z.i,i] <- calc_addSB(y, p=p, n_seedz=n_seedz, n_flz=n_flz, 
                            X.seed=X_seed[i,], X.fl=X_fl[i,]) * h
  Fs[z.i,z.i,i] <- (1 - p$p_emig) * Fb[z.i,z.i,i]
  Fs[1,z.i,i] <- (1 - p$p_emig) * Fb[1,z.i,i]
  Fs[z.i,1,i] <- calc_rcrSB(y, p) * p$p_est
  ...
  ## recruits: native
  p_est.i[i] <- ifelse(p$NDD, min(150/nSd[i], p$p_est), p$p_est)
  rcr.n <- round(nSd[i] * (1 - p$p_emig) * p$rcr_dir * p_est.i[i] +
                   B[i] * p$rcr_SB * p_est.i[i]) # ≥ 2 for IPMpack sd(rcr_z')
  if(rcr.n > 0) {
    rcr.i <- (1:rcr.n) + n_i[i]
    d[[i]][rcr.i,] <- NA
...
for(i in 1:n.cell) {
  ...
  sdd.j <- which(sdd[,,2,]==sdd.i, arr.ind=TRUE)
  p.ij <- sdd[,,1,][sdd.j]
  Fs[z.i,z.i,i] <- Fs[z.i,z.i,i] + 
    Reduce(`+`, map2(sdd.j[,3], p$p_emig*p.ij, ~(Fb[z.i,z.i,.x] * .y)))
  Fs[1,z.i,i] <- Fs[1,z.i,i] + Fb[1,z.i,sdd.j[,3]] %*% (p$p_emig*p.ij)
  ...
  # recruits: immigrants
  rcr.imm <- round(D[i] * p$rcr_dir * p_est.i[i])
  if(rcr.imm > 0) {
...
# density dependence
if(p$NDD) {
  lam.t <- p_est.t <- matrix(nrow=tmax-1, ncol=n.cell)
  for(i in 1:n.cell) {
    Nt[,,i,1] <- rpois((n+1)*(n+1), n_i)
    for(k in 1:(tmax-1)) {
      F.t <- Fs[,,i]
      # set p_est to density dependent value if that is < p$p_est
      p_est.t[k,i] <- min(150/(sum(F.t[z.i,] * Nt[z.i,,i,k])/p$p_est), p$p_est)
      # F.s[,,i] was already multiplied by p$p_est, so divide by that first
      F.t[z.i,] <- F.t[z.i,]/p$p_est * p_est.t[k,i]
      IPM.t <- Ps[,,i] + F.t
      Nt[,,i,k+1] <- IPM.t %*% Nt[,,i,k]
      lam.t[k,i] <- Re(eigen(IPM.t)$values[1])
    }
  }
}
IPMs <- Ps + Fs
...
lam.df %<>% 
  add_column(nSd=nSd, B0=B, B1=B_1, D=D, N=map_dbl(d, ~sum(!is.na(.$sizeNext))),
             N.surv=map_dbl(d, ~sum(.$surv, na.rm=T)),
             N.rcr=map_dbl(d, ~sum(is.na(.$size)))) %>%
  mutate(nSdStay=nSd*(1-p$p_emig), nSdLeave=nSd*p$p_emig) %>%
...
ggplot(lam.df, aes(x=lon, y=lat, fill=log(lam.tmax))) + geom_tile() +
  scale_fill_gradient2(low="darkblue", mid="white", high="darkred", midpoint=0)
ggplot(lam.df, aes(x=lon, y=lat, fill=log(Ntmax))) + geom_tile() +
...
sample.cells <- sample.int(n.cell, nPlots)
if(regPlots) {
...
library(dismo)
Mx.n <- 3000
Mx.P <- sample((1:nrow(lam.df))[lam.df$lambda > 1], Mx.n, replace=T)
