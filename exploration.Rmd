...
} else { cells <- sample(1:n_distinct(lc.df$CellID), subsamp, replace=F) }
n.cell <- length(cells)
...
for(i in 1:n.cell) {
  ...
  ## recruits
  rcrt.n <- max((sum(d[[i]]$seed, na.rm=T) * p$rcr_dir * p$p_est +
...
ggplot(lam.df, aes(x=x, y=y, fill=lambda>1)) + geom_tile()
par(mfrow=c(2,2))
