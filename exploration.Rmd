...
d <- sdd.j <- vector("list", length=n.cell)
B <- cbind(rpois(n.cell, n_i), matrix(0, nrow=n.cell, ncol=tmax))
nSd <- D <- p_est.i <- matrix(nrow=n.cell, ncol=tmax)
...
for(i in 1:n.cell) {
  ...
  E <- sim_expected(d[[i]], p, map(X, ~(.[i,])), n_z)
  d[[i]] <- sim_realized(d[[i]], 1:nrow(d[[i]]), E, p, lo, hi)
...
```{r O_setup}
O_n <- c(Mx=500, CA=20, IPM=20) # number of cells in sample
```

### Correlative: MaxEnt
```{r O_Mx}
library(dismo)
Mx.P <- sample((1:nrow(lam.df))[lam.df$lambda > 1], O_n$Mx, replace=T)
