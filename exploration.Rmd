...
sp_i <- list(p=list(s_z=c(-1.75, .75), # b1 + b2*z
                    ...
                    rcr_SB=0.01, # p(recruit from seedbank)
                    rcr_dir=0.95, # p(recruit directly)
                    s_SB=0.01 # p(survive in seedbank additional year)
                    ))
```
...
lambdas <- lambdas2 <- B_1 <- rep(NA, n.cell)
B <- rpois(n.cell, n_i)
...
for(i in 1:n.cell) {
  ...
  rcrt.n <- max((sum(d[[i]]$seed, na.rm=TRUE) * p$rcr_dir * p$p_est +
                ...
                2) # need ≥ 2 for IPMpack estimate of rcr_z'_mn, rcr_z'_sd
  ...
  ### fill P matrix
  P.mx <- F.mx <- matrix(0, ncol=n+1, nrow=n+1)
  for(k in 2:(n+1)) P.mx[k,2:(n+1)] <- G.mx[k-1,]*S.v
  P.mx[1,1] <- calc_staySB(p)
  
  ### fecundity
  F.mx[2:(n+1), 2:(n+1)] <- h*outer(y, y, calc_rcrDir, p=p, 
                                    n_seedz=n_seedz, n_flz=n_flz, 
                                    X.seed=X_seed[i,], X.fl=X_fl[i,])
  F.mx[1,2:(n+1)] <- calc_addSB(y, p=p, n_seedz=n_seedz, n_flz=n_flz, 
                                X.seed=X_seed[i,], X.fl=X_fl[i,])
  F.mx[2:(n+1),1] <- calc_rcrSB(y, p)
  ...
  if(regPlots) {
    ...
    w1 <- Re(eigen(IPM)$vectors[,1])[-1]; stable.dist1 <- w1/sum(w1)
  ...
  if(ipmPlots) {
    ...
    if(length(y) != nrow(IPM)) {
      x <- c(0, y)
    } else {
      x <- y
    }
    z.t$z_int <- findInterval(z.t$size, y, all.inside=TRUE)
    z.t$y <- y[z.t$z_int]
    totSeeds <- z.t %>% group_by(y) %>% summarise(seed=sum(seed, na.rm=TRUE))
    sens.mx <- sens(IPM); elas.mx <- elas(IPM)
    image.plot(x, x, t(P.mx), main=paste("P:", main.i), 
               xlab="Size (t)", ylab="Size (t+1)")
      points(sizeNext~size, data=z.t, pch=20, col=rgb(0,0,0,0.2))
      abline(a=0, b=1, col="white", lty=2)
    image.plot(x, x, t(F.mx), main=paste("F:", main.i), 
               xlab="Size (t)", ylab="Size (t+1)")
      points(totSeeds$y, totSeeds$seed/max(totSeeds$seed)*hi*.9, pch=19)
    image.plot(x, x, t(sens.mx),
               main=paste("Sens:", main.i), xlab="Size (t)", ylab="Size (t+1)")
    image.plot(x, x, t(elas.mx),
...
ggplot(lam.df, aes(x=x, y=y, fill=log(lambdas))) + geom_tile() +
  scale_fill_gradient2(low="darkblue", mid="white", high="darkred", 
                       midpoint=0, limits=log(range(c(lambdas, lambdas2))))
