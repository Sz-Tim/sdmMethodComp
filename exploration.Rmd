...
U <- fill_IPM_matrices(n.cell, tmax, n_i, n, z.rng, buffer=0.5, discrete=1,
                       p, n_z, n_x, X, sdd.pr, env.df)

#---- Realization: generate simulated data
S <- simulate_data(n.cell, tmax, n_i, z.rng, U$lo, U$hi, 
                   p, X, n_z, sdd, U$sdd.j)
...
lam.df %<>% 
  add_column(nSeed=S$nSd[,tmax], D=S$D[,tmax], 
  ...
  add_column(N.U=apply(U$Nt[,,,tmax],3,sum), lam.U=U$lam.t[,tmax-1])
...
ggplot(lam.df, aes(x=lon, y=lat, fill=log(lam.U))) + geom_tile() +
  scale_fill_gradient2(low="darkblue", mid="white", high="darkred", midpoint=0)
ggplot(lam.df, aes(x=lon, y=lat, fill=log(N.U)*(N.U>10))) + geom_tile() +
  scale_fill_gradient2(low="darkblue", mid="white", high="darkred", midpoint=1)
ggplot(lam.df, aes(x=lon, y=lat, fill=lambda>1)) + geom_tile()
ggplot(lam.df, aes(x=lon, y=lat, fill=N.S>1)) + geom_tile()
ggplot(lam.df, aes(x=lon, y=lat, fill=lam.U>1)) + geom_tile()
ggplot(lam.df, aes(x=lon, y=lat, fill=N.U>1)) + geom_tile() 
ggplot(lam.df, aes(x=lon, y=lat, fill=N.U>1 & lam.U<1)) + geom_tile() 
ggplot(lam.df, aes(x=lon, y=lat, fill=N.S>1 & lambda<1)) + geom_tile()
...
matplot(t(log(apply(U$Nt, 3:4, sum))), type="l", lty=1, col=rgb(0,0,0,0.1))
matplot(t(S$N_sim[,2:tmax]/S$N_sim[,1:(tmax-1)]), 
...
if(regPlots) {
  ...
  E.t <- xx %>% add_column(s=NA, g=NA, fl=NA, seed=NA)
  xxx <- seq(0, U$hi*.5, length.out=100)
  for(i in sample.cells) {
    z.t <- S$d[[i]][!is.na(S$d[[i]]$size) & S$d[[i]]$yr==tmax,]
    E.t <- sim_expected(E.t, 1:100, 1:100, E.t, p, map(X, ~.[i,]), n_z)
    main.i <- paste("Cell", i)
    ...
    plot(sizeNext~size, data=z.t, pch=20, col=rgb(0,0,0,0.2), ylim=c(U$lo,U$hi),
...
if(ipmPlots) {
  ...
  for(i in sample.cells) {
   ...
   image.plot(x, x, t(U$Ps[,,i]), main=paste("P matrix:", main.i), 
    ...
    points(sizeNext~size, data=z.t, col=rgb(1,1,1,1/(nrow(z.t)^.25)))
    abline(a=0, b=1, col="white", lty=2)
   image.plot(x, x, t(U$Fs[,,i]), main=paste("F matrix:", main.i), 
              xlab="Size (t)", ylab="Size (t+1)")
    points(totSeeds$y, totSeeds$seed/max(totSeeds$seed)*U$hi*.9, col="white")
...
for(i in IPM.P) {
  IPM.d[[i]] <- S$d[[i]] %>% filter(yr %in% O_yr$IPM) %>%
...
########
#---- Truth: use assigned slopes to fill IPM matrix
U.f <- fill_IPM_matrices(n.cell, tmax, n_i, n, z.rng, buffer=0.5, discrete=1,
                         p.ipm, n_z, n_x, X, sdd, env.df)

#---- Realization: generate simulated data
S.f <- simulate_data(n.cell, tmax, n_i, z.rng, U.f$lo, U.f$hi, 
                     p.ipm, X, n_z, sdd, U.f$sdd.j)

#---- Aggregate results
lam.df$lambda.f <- apply(U.f$IPMs, 3, function(x) Re(eigen(x)$values[1]))
lam.df %<>% 
  add_column(nSeed.f=S.f$nSd[,tmax], D.f=S.f$D[,tmax], 
             B0.f=S.f$B[,1], Btmax.f=S.f$B[,tmax+1], 
             N.S.f=map_dbl(S.f$d, ~sum(!is.na(.$sizeNext[.$yr==tmax]))),
             Surv.S.f=map_dbl(S.f$d, ~sum(.$surv[.$yr==tmax], na.rm=T)),
             Rcr.S.f=map_dbl(S.f$d, ~sum(is.na(.$size[.$yr==tmax])))) %>%
  mutate(nSdStay.f=nSeed.f*(1-p.ipm$p_emig), nSdLeave.f=nSeed.f*p.ipm$p_emig) %>%
  add_column(N.U.f=apply(U.f$Nt[,,,tmax],3,sum), lam.U.f=U.f$lam.t[,tmax-1])

########
## plots
########
ggplot(lam.df, aes(x=lon, y=lat, fill=lambda.f-lambda)) + geom_tile() +
  scale_fill_gradient2(low="darkblue", mid="white", high="darkred", midpoint=0)
ggplot(lam.df, aes(x=lon, y=lat, fill=lam.U.f-lam.U)) + geom_tile() +
  scale_fill_gradient2(low="darkblue", mid="white", high="darkred", midpoint=0)
...
ggplot(lam.df, aes(x=lon, y=lat, fill=(N.S.f-N.S)/N.S)) + geom_tile() +
  scale_fill_gradient2(low="darkblue", mid="white", high="darkred", midpoint=0)
ggplot(lam.df, aes(x=lon, y=lat, fill=(Surv.S.f-Surv.S)/Surv.S)) + geom_tile() +
  scale_fill_gradient2(low="darkblue", mid="white", high="darkred", midpoint=0)
ggplot(lam.df, aes(x=lon, y=lat, fill=(Rcr.S.f-Rcr.S)/Rcr.S)) + geom_tile() +
  scale_fill_gradient2(low="darkblue", mid="white", high="darkred", midpoint=0)
par(mfrow=c(2,2))
...
if(regPlots) {
  par(mfrow=c(3,2), mar=c(4,4,2,1))
  xx <- tibble(size=seq(U.f$lo, U.f$hi, length.out=100), 
               size2=size^2, size3=size^3)
  E.t <- xx %>% add_column(s=NA, g=NA, fl=NA, seed=NA)
  xxx <- seq(0, U.f$hi*.5, length.out=100)
  for(i in sample.cells) {
    z.t <- S.f$d[[i]][!is.na(S.f$d[[i]]$size) & S.f$d[[i]]$yr==tmax,]
    E.t <- sim_expected(E.t, 1:100, 1:100, E.t, p.ipm, map(X, ~.[i,]), n_z)
    E.S.t <- sim_expected(E.t, 1:100, 1:100, E.t, p, map(X, ~.[i,]), n_z)
    main.i <- paste("Cell", i)
    ...
    hist(with(S.f$d[[i]], sizeNext[is.na(size)]), col="grey", freq=F, 
         main=paste("Recruit sizes:", main.i), xlab="Recruit size (t+1)")
...
if(ipmPlots) {
  ...
  for(i in sample.cells) {
   ...
   image.plot(x, x, t(U.f$Fs[,,i]), main=paste("F matrix:", main.i), 
              xlab="Size (t)", ylab="Size (t+1)")
    points(totSeeds$y, totSeeds$seed/max(totSeeds$seed)*U.f$hi*.9, col="white")
