...
for(i in 1:n.cell) {
  ...
  Fb[1,z.i,i] <- calc_addSB(y, p=p, n_seedz=n_seedz, n_flz=n_flz, 
                            X.seed=X_seed[i,], X.fl=X_fl[i,])
  Fs[z.i,z.i,i] <- h * (1 - p$p_emig) * Fb[z.i,z.i,i]
  ...
  rcr.n <- max(ceiling(nSd[i] * (1 - p$p_emig) * p$rcr_dir * p$p_est +
                   B[i] * p$rcr_SB * p$p_est), 2) # ≥ 2 for IPMpack sd(rcr_z')
  rcr.i <- (1:rcr.n) + n_i[i]
  d[[i]][rcr.i,] <- NA
...
for(i in 1:n.cell) {
  ...
  Fs[z.i,z.i,i] <- Fs[z.i,z.i,i] + 
    h * Reduce(`+`, map2(sdd.j[,3], p$p_emig*p.ij, ~(Fb[z.i,z.i,.x] * .y)))
...
Mx.out <- maxent(x=lam.df[c("temp", "prec")], p=pres)
```
### Mechanistic: CA
```{r O_CA}
