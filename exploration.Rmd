...
sp_i <- list(p=list(s_z=c(-1.75, .75), # b1 + b2*z
                    ...
                    rcr_SB=0.4, # p(recruit from seedbank)
...
for(i in 1:n.cell) {
  ...
  ## recruits
  rcrt.n <- max((sum(d[[i]]$seed, na.rm=TRUE) * p$rcr_dir +
  ...
  #                        number=c(p$s_SB, p$rcr_SB*p$p_est, (1-p$rcr_dir)))
  ...
  # D.mx2@discreteTrans[1,1] <- p$s_SB 
  ...
  if(ipmPlots) {
    ...
    image.plot(x, x, t(IPM), main=main.i, xlab="Size (t)", ylab="Size (t+1)")
      points(sizeNext~size, data=z.t, pch=20, col=rgb(0,0,0,0.2))
      abline(a=0, b=1, col="white", lty=2)
    image.plot(P.mx2@meshpoints, P.mx2@meshpoints, t(IPM2), 
               main=main.i, xlab="Size (t)", ylab="Size (t+1)")
      points(sizeNext~size, data=z.t, pch=20, col=rgb(0,0,0,0.2))
      abline(a=0, b=1, col="white", lty=2)
