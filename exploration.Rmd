...
sp_i <- list(p=list(s_z=c(-1.75, .75), # b1 + b2*z
                    ...
                    rcr_dir=0.9, # p(recruit directly)
...
for(i in 1:n.cell) {
  ...
  rcr.n <- ceiling(nSd[i] * (1 - p$p_emig) * p$rcr_dir * p_est.i[i] +
...
for(i in 1:n.cell) {
  ...
  # recruits: immigrants
  rcr.imm <- ceiling(D[i] * p$rcr_dir * p_est.i[i])
...
ggplot(lam.df, aes(x=lon, y=lat, fill=(D-nSdStay)/nSdStay)) + geom_tile() +
  scale_fill_gradient2(low="darkblue", mid="white", high="darkred", midpoint=0)
