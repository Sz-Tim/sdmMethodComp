...
sp_i <- list(p=list(s_z=c(-1.75, .75), # b1 + b2*z
                    ...
                    fl_x=c(1, -.01, -1, -.01, .05), # b1*x1 + ...
                    seed_z=c(2, 0.5, -.03), # b1 + b2*z + b2*z^2
                    seed_x=c(1, -.1, 1, -.1, .05), # b1*x1 + ...
...
p <- sp_i$p
IPMs <- array(0, dim=c(n+1, n+1, n.cell))
...
for(i in 1:n.cell) {
  ...
  F.mx[z.i,z.i] <- h*outer(y, y, calc_rcrDir, p=p, n_seedz=n_seedz, n_flz=n_flz, 
                           X.seed=X_seed[i,], X.fl=X_fl[i,])
  F.mx[1,z.i] <- calc_addSB(y, p=p, n_seedz=n_seedz, n_flz=n_flz, 
                            X.seed=X_seed[i,], X.fl=X_fl[i,])
...
if(regPlots) {
  ...
  for(i in 1:n.cell) {
    ...
    plot(sizeNext~size, data=z.t, pch=20, col=rgb(0,0,0,0.2), 
      ...
      points(z.t$size, E_grow, cex=0.2, col="red"); abline(a=0, b=1, lty=2)
    ...
    plot(y, stable.dist1, ylab="", type="l", lwd=2, col="red", 
