...
lc.df <- read_csv("data/landcover_5km.csv") %>% 
  ...
  mutate(x=as.integer(as.factor(lc.df$left)),
...
env.rct <- as.tibble(expand.grid(x=1:max(lc.df$x),
  ...
  mutate(temp=c(scale(lc.df$bio1_mean))[match(.$x_y, lc.df$x_y)],
         ...
         prec2=prec^2,
         pOpn=c(scale(lc.df$nlcd1_mean))[match(.$x_y, lc.df$x_y)],
         pOth=c(scale(lc.df$nlcd2_mean))[match(.$x_y, lc.df$x_y)],
         pDec=c(scale(lc.df$nlcd3_mean))[match(.$x_y, lc.df$x_y)],
         pEvg=c(scale(lc.df$nlcd4_mean))[match(.$x_y, lc.df$x_y)],
         pMxd=c(scale(lc.df$nlcd5_mean))[match(.$x_y, lc.df$x_y)],
...
sdd.pr <- sdd_set_probs(ncell=n.cell, lc.df=env.rct, lc.col=8:12,
                        g.p=list(sdd.max=4, sdd.rate=0.8, bird.hab=rep(1,5)))
...
for(i in 1:n.cell) {
  ...
  B_1[i] <- B[i] * p$s_SB * (1 - p$rcr_SB) + 
    sum(d[[i]]$seed, na.rm=T) * (1 - p$rcr_dir) * p$s_SB
  ## recruits
  rcrt.n <- max(ceiling((sum(d[[i]]$seed, na.rm=T) * p$rcr_dir * p$p_est +
